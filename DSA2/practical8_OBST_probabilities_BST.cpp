/*Title: Given sequence k = k1  …  kn of n sorted keys, with a search probability pi for each key ki .
 Build the Binary search tree that has the least search cost given the access probability for each key.*/
#include<iostream>
using namespace std;
void con_obst(void);       //a[20]=successful probability search
void print(int,int);           //b[20]=unsuccessful probability search
float a[20],b[20],wt[20][20],c[20][20];    // wt=weight matrix,c=cost matrix
int r[20][20],n;
int main()
  {
	int i;
...
void con_obst(void) //construct OBST
{
	int i,j,k,l,min;
	for(i=0;i<n;i++)
	  { //Initialisation
